function [adj_bboxes] = adjust_bb_basedon_label(ori_bbox) % top left bottom right
label = ori_bbox(1);
bbox = ori_bbox(2 : 5);
label_num_1 = 1; % 1
label_dot   = 83; % .
...
if label == 98
    adj_bboxes = [label, adj_bboxes, ori_bbox(6), ori_bbox(7)];
else
    ori_adj_bboxes = adj_bboxes;
    adj_bboxes = zeros(size(ori_adj_bboxes, 1), 7);
    for bid = 1 : size(ori_adj_bboxes, 1)
        adj_bboxes(bid, :) = [label, ori_adj_bboxes(bid, :), (ori_adj_bboxes(bid, 1) + ori_adj_bboxes(bid, 3)) / 2, ...
            (ori_adj_bboxes(bid, 2) + ori_adj_bboxes(bid, 4)) / 2];
    end
end
end
