        ...
        for k = 1 : size(bboxes_j, 1)
            if label ~= 98
                fprintf(fp_adjt_bb, '%.0f %.0f %.0f %.0f %.0f %g %g\n',  ...
                    label, bboxes_j(k, 1), bboxes_j(k, 2), bboxes_j(k, 3), bboxes_j(k, 4), ...
                    roundn((bboxes_j(k, 1) + bboxes_j(k, 3)) / 2, -1), ...
                    roundn((bboxes_j(k, 2) + bboxes_j(k, 4)) / 2, -1)); % top left bottom right
            else
                fprintf(fp_adjt_bb, '%.0f %.0f %.0f %.0f %.0f %g %g\n',  ...
                    label, bboxes_j(k, 1), bboxes_j(k, 2), bboxes_j(k, 3), bboxes_j(k, 4), ...
                    orig_label_bb(j, 6), orig_label_bb(j, 7));
            end
        end
    end
    fclose(fp_adjt_bb);
